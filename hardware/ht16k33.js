...
class HT16K33 {
	constructor(address, i2c) {
		this.d_i2c = i2c;
		this.d_addr = address;
		this.d_buffer = Buffer.alloc(16);
	}

	begin() {
		// Initialize driver with LEDs enabled and all turned off
		// Wake up the oscillator
		this.d_i2c.writeSync(this.d_addr, HT16K33_SYSTEM_SETUP | HT16K33_OSCILLATOR, EMPTY_BUFFER);
	...
	set_blink(frequency) {
		...
		this.d_i2c.writeSync(this.d_addr, HT16K33_BLINK_CMD | HT16K33_BLINK_DISPLAYON | frequency, EMPTY_BUFFER);
	}
	...
	set_brightness(brightness) {
		...
		this.d_i2c.writeSync(this.d_addr, HT16K33_CMD_BRIGHTNESS | brightness, EMPTY_BUFFER);
	}
	...
	write_display() {
		for (var i = 0; i < this.d_buffer.size; i++) {
